...
All examples assume the working directory is `adc/`.  `make build` writes `.bin/adc` and `.bin/adcengine`.  The `adjservices` repository contains Clerk processes, agent launchers, MCP adapters, attestation, and deployment material.
