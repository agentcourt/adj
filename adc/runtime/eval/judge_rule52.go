...
func RunJudgeRule52(ctx context.Context, opts JudgeRule52Options) (resultValue JudgeRule52Summary, returnErr error) {
	...
	summary := newJudgeRule52Summary(opts, promptVariant, resultsPath, summaryPath, runID)
...
func RescoreJudgeRule52(opts JudgeRule52RescoreOptions) (resultValue JudgeRule52Summary, returnErr error) {
	...
	source, sourceSummaryPath, err := loadJudgeEvalSourceSummary(opts.ResultsPath, opts.SourceSummaryPath, "judge_rule52")
	if err != nil {
		return JudgeRule52Summary{}, err
	}
	if err := validateJudgeEvalRescoreRows(opts.ResultsPath, source, results, judgeRule52ResultIdentity, validateJudgeRule52RescoreResult); err != nil {
		return JudgeRule52Summary{}, err
	}
...
func newJudgeRule52Summary(opts JudgeRule52Options, promptVariant judgeRule52PromptVariant, resultsPath string, summaryPath string, runID string) JudgeRule52Summary {
...
func runJudgeRule52Fixture(
	...
	responseClient := judgeEvalResponseClient(opts.DryRun, client, dryRunJudgeRule52Response(fixture))
	...
	result := scoreJudgeRule52Response(fixture, opts.Model, opts.DryRun, state, execution.View, execution.Opportunity, decision.Input, decision.ScoringResponse)
...
func readJudgeRule52Results(path string) (resultValue []JudgeRule52Result, returnErr error) {
	return readJudgeEvalJSONL[JudgeRule52Result](path, judgeEvalRequiredResultFields(
		"tier", "issue_family", "case_theme", "complaint_text", "answer_text", "plaintiff_theory", "defendant_theory",
		"admitted_evidence", "plaintiff_closing", "defendant_closing", "expected_winner", "required_concepts",
		"expected_reason_tags", "severity", "winner_correct", "amount_correct", "required_correct",
		"prohibited_correct", "separation_correct", "matched_reason_tags",
	)...)
}

func judgeRule52ResultIdentity(result JudgeRule52Result) judgeEvalResultIdentity {
	return judgeEvalResultIdentity{
		...
		PromptSource:        resultJudgeRule52PromptSource(result),
		PromptName:          resultJudgeRule52PromptName(result),
...
func validateJudgeRule52RescoreResult(result JudgeRule52Result) error {
	return (JudgeRule52Fixture{
		...
		PlaintiffTheory:    result.PlaintiffTheory,
		DefendantTheory:    result.DefendantTheory,
		AdmittedEvidence:   result.AdmittedEvidence,
		ExcludedEvidence:   result.ExcludedEvidence,
		PlaintiffClosing:   result.PlaintiffClosing,
		DefendantClosing:   result.DefendantClosing,
		ExpectedWinner:     result.ExpectedWinner,
		ExpectedAmountText: result.ExpectedAmountText,
	...
	return openai.Response{
		...
		ToolCalls: []openai.ToolCall{{
			...
			Arguments: map[string]any{
				"text":        opinion,
				"verdict_for": normalizeParty(f.ExpectedWinner),
			},
		}},
	}
}
