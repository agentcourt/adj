...
var fixedDefinitions = []definition{
	...
	def(RuntimeSystemID, "runtime/system.md", "Role: {{ROLE}}\nRole prompt preamble: {{PREAMBLE}}\nInstructions: {{INSTRUCTIONS}}\nAllowed court actions: {{ALLOWED_ACTIONS}}\nUse the listed court actions with their required payloads to make a filing or decision. Use available support tools to inspect the record. For investigation, use the analysis, execution, and computer-use tools supplied by your harness under the case's search setting. Install tools when needed and keep useful work in the retained workspace. When ready, submit one permitted court action.\nCurrent view:\n{{VIEW}}", []string{"{{ROLE}}", "{{PREAMBLE}}", "{{INSTRUCTIONS}}", "{{ALLOWED_ACTIONS}}", "{{VIEW}}"}),
	def(RuntimeOpportunityID, "runtime/opportunity.md", "Current opportunity:\n{{ACTOR_MESSAGE}}\nObjective: {{OBJECTIVE}}\nPhase: {{PHASE}}\nAllowed actions: {{ALLOWED_ACTIONS}}\nReference tools: {{REFERENCE_TOOLS}}\nConstraints: {{CONSTRAINTS}}\nUse constraints.by_tool[tool_name] when present. Otherwise use the opportunity-level constraints. When import_case_file is allowed, import source documents or analysis files needed to support the pending filing. After an import, obtain the next opportunity and complete the filing or pass when permitted.\nPass action: {{PASS_ACTION}}", []string{"{{ACTOR_MESSAGE}}", "{{OBJECTIVE}}", "{{PHASE}}", "{{ALLOWED_ACTIONS}}", "{{REFERENCE_TOOLS}}", "{{CONSTRAINTS}}", "{{PASS_ACTION}}"}),
	...
	def(ReportSummarySystemID, "report/summary/system.md", "Summarize each side's civil-trial arguments precisely from the supplied record. Use case file for a registered document and exhibit for a document offered as an exhibit. State admission or exclusion according to the recorded ruling. Describe document production as disclosure to the opposing party. Attribute verification work and its conclusions to the participant who reported them.", nil),
