...
| `adc` | `core_command` defaults to `adc`, while `core_working_directory` is optional.  `mcp_command` defaults to `adc-mcp`, and `mcp_working_directory` defaults to the core working directory.  The object accepts plaintiff and defendant profiles and defaults `trial_mode` to `auto`, `auto_lawyers` to `both`, and `web_search` to true.  `court_submission_error_limit` defaults to 10.  It also accepts optional MCP address, prompt, and timeout fields. |
...
ADC's `court_submission_error_limit` applies to locally launched Pi lawyers and jurors.  It counts court submissions rejected inside Pi and resets for each opportunity.  A positive integer overrides 10, zero selects 10, and negative values are rejected.  The [ADC manual](adc/manual.md#local-pi-submission-errors) defines the counted failures and resulting participant-failure handling.
