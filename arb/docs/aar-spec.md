...
`run.json` records the final state, resolution, council roster, votes, admitted evidence, final reason, generated artifacts, the council cost reported by successful OpenRouter generation lookups, and the last provider error class when a provider failure removed a council member.  `state.json`, `certificate.json`, `events.ndjson`, `evidence-manifest.json`, `transcript.md`, and `digest.md` supply the other durable views.  Each provider-failure removal event records its class, while preflight failures preserve their typed class through the process error and command summary.
