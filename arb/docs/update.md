...
## Prompt ownership and resolution

The AAR core now owns every model-facing prompt used for attorney turns, direct and API council turns, council preflight, observer access, correction requests, and tool descriptions.  The prompt catalog and its compiled fallbacks live in `arb/runtime/proceeding`, while the conventional editable files live under `prompts/arb`.  Neither resolution nor role execution imports or calls `adjservices`.

The repeatable `--prompt-file ID=PATH` option and `Options.PromptFiles` map provide partial prompt replacement.  `--prompt-dir` supplies a complete set and fails when any non-individually-overridden catalog file is absent.  Resolution then checks the working-directory convention before selecting the concise compiled fallback.

The resolver validates each source and its allowed `{{TOKEN}}` sequences before inserting runtime values.  Wrapper prompts compose standing, common, capability, workspace, limits, phase, and final attorney components, while council templates cover the system prompt, persona, and later request or correction instructions.  Every distinct model-facing tool description has its own catalog entry; tool schemas, validation errors, and record rendering remain code-owned runtime data.
...
The direct AAR service leaves that output directory to the core and stores new child standard streams beneath `RegistryDir/logs/<case_id>/`.  Its artifact API retains the logical `service-logs/aar.stdout` and `service-logs/aar.stderr` names and accepts only exact registry paths recorded for the case.  The service reserves case identifiers against active and persisted direct and Clerk records, creates confined log directories and files exclusively, publishes its record through a unique temporary file, and verifies directory identity before failed-create cleanup.  Every direct terminal consumer uses one reader that requires matching case and run identifiers and consistent top-level and final Lean case statuses.  The `aar-run` launcher owns an outer service run directory, passes its dedicated `aar-output` child to the core, and keeps component logs and `local-run.json` outside that child.  It requires empty core and log children, rejects symbolic-link substitutions, and rejects a child whose directory identity changes during validation.  Each container-backed participant writes its complete runtime container identifier to a private cidfile, and cleanup removes only that verified identifier.  Local and attested Clerk readers resolve core artifacts through the nested core path; an outer-root `run.json` has no core-result role.  The reporter descends through the outer service records and reports their failure or incomplete state when no nested core marker exists.  For a manual role, the launcher writes a mode-`0600` remote-lawyer skill containing that role's MCP capability and removes it during cleanup.  Attested event reads prefer the nested core copy from a verified success archive and otherwise use the live or downloaded top-level event object; a partial event copy remains diagnostic.  The attested archive retains the launcher summary, component logs, and core record and excludes Pi homes, staged Codex homes, and remote-lawyer skills.  The Python driver publishes an extracted service root only after safe extraction into a sibling temporary directory, cleans remote temporary directories after failed transfers, and preserves cleanup errors.  The exec entrypoint stops before upload or manifest creation when archive creation, hashing, or sizing fails.  A trusted attested result comes only from `aar-run/aar-output/run.json`, matches the Clerk case and run identifiers, and pairs `ok` with `closed` or `failed` with `failed`.  A partial attested archive cannot complete a case.
...
### Completed AARD mapping

The `arbd` tree now carries the corresponding evidence storage, Lawyer and Council APIs, case packets, replay certificates, and Lean proof results.  Its [engine](../../arbd/engine/Main.lean) has an initial evidence catalog, exact `OpportunityAuthority`, record-integrity predicates, submitted lineage, and replay-time offer chronology.  Its [proceeding runtime](../../arbd/runtime/proceeding/run.go) captures initial evidence before council sampling and initialization, serializes mutable case state through one mutex, bounds engine calls, and publishes submitted evidence through the same ordered transaction used by AAR.

| AAR source | AARD target | Adaptation |
|---|---|---|
| [Engine model and validation](../engine/Main.lean) | `arbd/engine/Main.lean` | Added commitments, authority, strict optional parsing, lineage, offer validation, report byte checks, and surrebuttal supplemental materials while retaining numeric `CouncilAnswer`. |
| [Record-integrity proofs](../engine/Proofs/RecordIntegrity.lean) | `arbd/engine/Proofs/RecordIntegrity.lean` | Uses answer-specific branches and proves preservation for every AARD action. |
| [Replay and certificate facts](../engine/Proofs/Replay.lean) | `arbd/engine/Proofs/Replay.lean`, `CertificateFacts.lean` | Establishes authority-conforming replay, offer chronology, final-state integrity, and fixed-catalog facts for closed and failed certificates. |
| [Lean process adapter](../runtime/lean/engine.go) | `arbd/runtime/lean/engine.go` | Uses contexts, deadlines, process-group cleanup, exit-1 rejection semantics, and joined parse and process errors. |
| [Proceeding runtime](../runtime/proceeding/run.go) | `arbd/runtime/proceeding/run.go`, `types.go`, `helpers.go`, `case_packet.go` | Ingests the verified catalog before council sampling, provider preflight, and initialization, excludes the configured complaint by file identity, uses one case mutex, and captures an owned final snapshot. |
| [Evidence custody](../runtime/proceeding/evidence.go) | `arbd/runtime/proceeding/evidence.go`, `lawyerapi.go` | Uses single-descriptor capture and reads, lineage derivation, chunk reconciliation, candidate manifest publication, and one commit path. |
| [Role API rules](../runtime/proceeding/roleapi_errors.go) | `arbd/runtime/proceeding/lawyerapi.go`, `councilapi.go`, `roleapi_errors.go` | Enforces strict schemas, error ownership, shared notifications, accepted-state checks, and deadline behavior. |
| [Case-packet publication](../runtime/proceeding/case_packet.go) | `arbd/runtime/proceeding/case_packet.go` | Enforces source identity checks, prepublication packet hashing, paired reservations, and rollback. |

AARD retained `CouncilAnswer` values from 0 through 100 and aligned Lean surrebuttal handling with ARAP and the Go API.  The approved outcomes below define the durable formats used by the completed port.  Later changes to these choices require a new schema or an explicit migration decision.

| AARD decision | Approved outcome |
|---|---|
| Surrebuttal materials | Lean permits offered evidence, technical reports, and submitted evidence during surrebuttal, matching ARAP and Go. |
| Certificate schema | Authority-bearing replay uses `aard.replay-certificate.v1`; the state schema remains `v1`. |
| Evidence identifiers | The runtime retains the digest-prefixed `ev_<sha[:12]>_<slug>` identifier and adds canonical commitments and collision checks around it. |
| Failed council members | The runtime retains `fail_opportunity` and proves its interaction with record actions without adding a member-removal record. |

Keeping the answer model unchanged isolated record work from substantive judgment semantics.  The real-engine coverage includes a derived submission and a later offer, exercising the catalog, prior history, lineage, source-state chronology, runtime publication, and replay together.  It compares the accepted engine state, stored evidence manifest, and replay certificate for the same parent and offer identifiers.
