...
| Replay certificates | `checkReplayCertificate_terminal_facts`, `checkReplayCertificate_status_closed_facts`, `checkReplayCertificate_status_failed_facts` | Accepted terminal certificates inherit exact replay, opportunity-authority conformance, reachability, bounded length, decision-summary replay, and either closed-case soundness or an accounted failed-opportunity record. |
...
The Lean certificate package covers both terminal outcomes.  Closed packets carry exact replay, authority conformance, reachability, terminal accounting, ordered merits completion, filing counts, decision-rule facts, matched-case closed-resolution agreement, decision-summary replay, and resolution-specific soundness.  Failed packets carry exact replay, authority conformance, reachability, the initialized action-length bound, decision-summary replay, and an `opportunity_failed` record identifying a plaintiff or defendant role and the failed phase.  The matched-case theorem compares closed-resolution summaries after the current-round vote multiset, seated count, deliberation round, required-vote value, and max-round value have been fixed.
