...
func promptDefinitions() []mcpbridge.PromptDefinition {
	definitions := arbitrationBasePromptDefinitions(
		...
		map[string]string{
			...
			"get_case_result":         "Return final case results, including council votes and rationales, or pending status.",
			...
			"submit_council_vote":     "Submit one council vote for the current deliberation opportunity.",
		},
	)
	return definitions
}

func arbitrationBasePromptDefinitions(session string, descriptions map[string]string) []mcpbridge.PromptDefinition {
	definitions := []mcpbridge.PromptDefinition{
		{ID: PromptSessionInstructions, RelativePath: "mcp/session.md", Fallback: session, Tokens: []string{"{{CASE_ID}}", "{{ASSIGNMENT_TYPE}}", "{{PRINCIPAL_ID}}"}},
