...
func TestCouncilPostRejectsTrailingJSONValueWithoutCountingAttempt(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			body: `{"case_id":"arb-1","member_id":"C1","opportunity_id":"deliberation:1:C1","tool":"get_case","arguments":{}} {}`,
		},
		{
			name: "fail",
			path: councilAPIBasePath + "/fail",
			body: `{"case_id":"arb-1","member_id":"C1","opportunity_id":"deliberation:1:C1"} {}`,
		},
	}
...
func TestCouncilDoRejectsUnknownToolArgumentFields(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "vote", tool: "submit_council_vote", arguments: map[string]any{"vote": "demonstrated", "rationale": "Sufficient.", "unexpected": true}, unknown: "unexpected"},
		{name: "caller member id", tool: "submit_council_vote", arguments: map[string]any{"vote": "demonstrated", "rationale": "Sufficient.", "member_id": "C2"}, unknown: "member_id"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			api, turn := testCouncilAPIWithTurn(t)
			status, got := callCouncilAPIDo(t, api, map[string]any{
...
func TestCouncilSubmitVoteEngineCallCrossingDeadlineRecordsDeadlineFailure(t *testing.T) {
...
func assertCouncilAPIDeadlineFailure(t *testing.T, api *councilAPIServer, turn *councilTurn) {
	...
	if votes := mapList(mapAny(api.rc.state["case"])["council_votes"]); len(votes) != 0 {
