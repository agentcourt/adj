...
var promptDefinitions = func() []promptDefinition {
	definitions := []promptDefinition{
		...
		{id: promptAttorneyClosings, relativePath: "attorney/phase/closings.md", fallback: "Synthesize the admitted record and explain whether the proposition meets the evidence standard.", tokens: attorneyPhaseTokens()},
		{
		...
		},
		{
			...
			fallback:     "You are council member {{MEMBER_ID}} in deliberation round {{DELIBERATION_ROUND}}. Decide whether the proposition is demonstrated under the stated evidence standard.\n\nProposition: {{PROPOSITION}}\nEvidence standard: {{EVIDENCE_STANDARD}}\n{{PERSONA_SECTION}}\nRecord:\n{{RECORD}}",
			tokens:       []string{"MEMBER_ID", "DELIBERATION_ROUND", "PROPOSITION", "EVIDENCE_STANDARD", "PERSONA_SECTION", "RECORD", "OPPORTUNITY_ID", "OBJECTIVE"},
		},
		{
		...
		},
		{
			...
			fallback:     "You are being checked for availability as an Agent Arbitration council member. Reply with the exact word ready.",
...
func attorneyPhaseTokens() []string {
	return []string{"ROLE", "PHASE", "OBJECTIVE", "OPPORTUNITY_ID", "PROPOSITION", "EVIDENCE_STANDARD", "DECISION_TOOLS"}
}
...
func (cfg Config) resolvePromptSources() (map[string]string, error) {
	...
	for _, definition := range promptDefinitions {
		...
		specs = append(specs, promptfile.Spec{
			ID:               definition.id,
			Name:             definition.id,
			ConventionalPath: filepath.Join("prompts", "arb", definition.relativePath),
