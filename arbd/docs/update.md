...
The AARD core now owns every model-facing prompt used for attorney turns, direct and API council turns, council preflight, observer access, correction requests, and tool descriptions.  The prompt catalog and its compiled fallbacks live in `arbd/runtime/proceeding`, while the conventional editable files live under `prompts/arbd`.  Neither resolution nor role execution imports or calls `adjservices`.
