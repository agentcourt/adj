	...
	return proceeding.ReplayCertificate{
		...
		InitializeRequest: proceeding.ReplayInitializeRequest{
			State: map[string]any{
				"case":          map[string]any{"case_id": "cert-case", "phase": "draft", "status": "active"},
				"state_version": 0,
			},
...
request=$(cat)
case "$request" in
  *initialize_case*) printf '%s\n' '{"ok":true,"state":{"case":{"case_id":"cert-case","phase":"openings","status":"active"},"state_version":1}}' ;;
  *record_opening_statement*) printf '%s\n' '{"ok":true,"state":{"case":{"case_id":"cert-case","phase":"closed","status":"closed","council_answers":[{"round":1,"member_id":"C1","answer":72,"rationale":"Reason."}]},"state_version":2}}' ;;
