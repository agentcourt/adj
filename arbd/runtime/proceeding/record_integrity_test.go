...
func TestSubmittedEvidenceLineageDerivesParentCommitment(t *testing.T) {
	...
	if payload["parent_evidence_id"] != parent.EvidenceID || payload["parent_sha256"] != parent.SHA256 {
	...
	rc.evidenceByID["private"] = EvidenceMeta{EvidenceID: "private", RecordVisibility: "system_private", AdmissibilityStatus: "case_packet"}
	bad = cloneMap(params)
	bad["parent_evidence_id"] = "private"
	if _, _, err := rc.prepareSubmittedEvidence(Opportunity{Role: "plaintiff", Phase: "arguments"}, bad); err == nil {
		t.Fatal("private lineage parent was accepted")
	}
}

func TestDirectEvidencePublicationCanRetryAfterManifestFailure(t *testing.T) {
	dir := t.TempDir()
...
func TestUploadPublicationCanRetryAfterManifestFailure(t *testing.T) {
	dir := t.TempDir()
	api, turn := newSubmissionTransactionTestAPI(t, dir)
...
func newSubmissionTransactionTestAPI(t *testing.T, dir string) (*lawyerAPIServer, *lawyerTurn) {
	...
	rc := &runContext{
		cfg: Config{
			CaseID: "arbd-1", OutputDir: dir, Policy: policy, Runtime: DefaultRuntimeLimits(),
...
func TestSubmittedEvidenceLineagePersistsInStateManifestEventAndCertificateAction(t *testing.T) {
	...
	parent := EvidenceMeta{
		...
		AdmissibilityStatus: "case_packet", RecordVisibility: "council_visible",
	}
