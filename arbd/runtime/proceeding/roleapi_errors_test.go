...
func TestRoleAPIParticipantInputConsumesOneAttempt(t *testing.T) {
	...
	t.Run("council answer outside range", func(t *testing.T) {
		...
		status, body := callCouncilAPIDo(t, api, map[string]any{
			...
			"arguments": map[string]any{
				"answer":    101,
				"rationale": "Outside the accepted range.",
			},
		})
		if status != http.StatusOK {
			t.Fatalf("status = %d, want %d", status, http.StatusOK)
...
func TestCouncilAcceptedAnswerEventFailureCompletesWithoutAttemptCharge(t *testing.T) {
	api, turn := testCouncilAPIWithTurn(t)
	outputFile := filepath.Join(t.TempDir(), "output-file")
	if err := os.WriteFile(outputFile, []byte("not a directory"), 0o644); err != nil {
		t.Fatalf("write output blocker: %v", err)
	...
	if len(api.rc.certificateActions) != 1 || len(api.rc.events) != 1 {
		t.Fatalf("accepted answer commit = %d actions and %d events, want 1 and 1", len(api.rc.certificateActions), len(api.rc.events))
	}
	answers := mapList(mapAny(api.rc.state["case"])["council_answers"])
	if len(answers) != 1 || intNumber(answers[0]["answer"]) != 72 {
		t.Fatalf("committed answers = %#v, want one answer of 72", answers)
	}
}
