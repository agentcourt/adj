...
import (
	...
	"github.com/agentcourt/adj/runtime/localrun/arbd"
)
...
func runLocal(ctx context.Context, args []string, stdout io.Writer, stderr io.Writer) error {
	...
	mcpCommand := fs.String("mcp-bin", localrun.DefaultMCPCommand, "ARBD MCP executable")
	mcpWorkingDir := fs.String("mcp-working-dir", "", "Optional working directory for the ARBD MCP process; defaults to --aard-working-dir")
	...
	promptDir := fs.String("prompt-dir", "", "Complete ARBD prompt directory")
	fs.Var(&promptFiles, "prompt-file", "Prompt override as ID=PATH; repeat for a partial set")
	launcherPromptDir := fs.String("launcher-prompt-dir", "", "Complete ARBD launcher prompt directory")
	...
	autoLawyers := fs.String("auto-lawyers", localrun.DefaultAutoLawyers, "Lawyers started by aard-run: both, plaintiff, defendant, or none")
	...
	fs.Usage = func() {
		fmt.Fprintf(flagOutput, "Usage: aard-run [EXAMPLE] [options]\n\n")
		fs.PrintDefaults()
	}
	...
	help, err := cliio.Parse(fs, flagArgs, flagOutput)
	if err != nil {
		return err
	}
	if help {
