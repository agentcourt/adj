...
## Pi history compatibility and ADC turn limit: 2026-09-19

The user approved the gateway corrections, 500 generated turns, live testing, a fresh full ADC case with the existing nine-juror/six-vote settings, and completion of the marXiv report.  The 25-minute turn deadlines remain unchanged.

The gateway comparison now normalizes whitespace-only assistant text when tool calls are present.  If the original provider argument string is malformed JSON, it accepts Pi's parsed arguments while comparing the remaining message fields.  The shared executor retains the original provider response for continuation.  Valid original arguments still require semantic equality.  The installed Pi source for this behavior is `@mariozechner/pi-ai/dist/providers/openai-completions.js` and `@mariozechner/pi-ai/dist/utils/json-parse.js` beneath the global Pi installation.

- [x] Implement history normalization and the generated 500-turn limit.
- [x] Run focused Go tests and live Pi/MCP juror tests using the affected configurations.
- [x] Verify the live record and review documentation.
- [x] Commit and push the verified corrections.
- [ ] Run the full case with fresh lawyer state, then complete and submit the report.

The first live test, `adc/out/gateway-live-20260919-01/`, used the installed DeepSeek V3.2/AtlasCloud and Qwen3 30B A3B Instruct/SiliconFlow configurations.  DeepSeek submitted its questionnaire.  Qwen reproduced the malformed argument string from the failed full case, and its subsequent requests passed the gateway after Pi parsed it.  Qwen then called nonexistent tools, submitted malformed arguments to the Pi proxy, and repeated context reads without submitting a questionnaire.  The test was stopped after its 27 completed responses.  It verifies continued requests through the corrected gateway but provides no successful Qwen questionnaire.  The Qwen model behavior remains a limitation of that configuration.

The second live test, `adc/out/gateway-live-20260919-02/`, assigned three DeepSeek configurations and three Sonnet 4.6 controls.  It completed six questionnaires and follow-up questions for one juror from each model.  Its digest contains all eight juror process logs.  Certificate verification passed all 24 transitions through local leanrunner with 4 GiB memory high, 6 GiB maximum, 1 GiB swap, 100% CPU, and a 900-second timeout.  The command was `adc/.bin/adc verify-certificate --dir adc/out/gateway-live-20260919-02/adc-output`, from the repository root.  Both tests left no container or staged authentication/MCP configuration file.  No Lean source or proof changed.

Focused gateway and case-generation tests passed.  ADC runtime tests, the ADC launcher and command tests, the unified command tests, and ARB/AARD launcher tests passed.  Affected vet checks and `git diff --check` passed.  Live juror request logs retain token usage but omit dollar totals.  The exact whitespace-only response from the stopped case is covered by the focused comparison test.  The live tests did not reproduce that text pattern.

Revision `cfdd312` was committed and pushed to `origin/main`.  The fresh full case began at 04:28:29 UTC under `adc/out/ex04-20260919-02/`, using run ID `ex04-20260919-02`.  Its generated scenario confirms 500 turns, and its runtime confirms 1,500-second model and participant deadlines.  Both new Pi sessions record provider `openai-codex`, model `gpt-5.6-sol`, and `xhigh`.  The initial document matches the original example byte for byte.  The earlier manuscript is preserved beside the stopped run as `adc/out/ex04-20260919-01/report-draft.tex`.

Both technical reports were accepted by 04:50 UTC.  The plaintiff imported Syria's official UN letter A/79/687–S/2024/888, and the defense imported Israel's S/2024/887.  Both stored PDFs match their workspace originals.  The lawyers installed PDF extraction tools in their containers and used search, HTTP downloads, and local text extraction.  Interrogatory answers narrowed the disagreement to offensive character and intended control, with deployment and continued presence largely accepted.  The plaintiff's claimed reporting consensus expressly concerns the reported acts rather than agreement over the ultimate market definition.  Its research included an eleven-minute, fifty-six-second gap between work notes.  The defense supplied substantive notes during its shorter research interval.

By 05:01 UTC, the plaintiff had answered the defense's seven production requests and supplied its retained source pages in `file-0004`.  The response identifies republications, inaccessible originals, and absent operational and geospatial material.  The defense's response to eight requests remains in progress.  The incomplete manuscript now follows this fresh record through the plaintiff's production.  No code, prompt, pool, or case input has changed during the run.  The lawyer logs recorded 188 completed responses and $32.14 in token-price estimates at 05:02 UTC, excluding court requests and search subrequests.

The defense completed production at 05:04 UTC.  The plaintiff admitted all 21 defense admission requests with limited qualifications.  The defense admitted or qualified the first 23 plaintiff requests and denied the final two ultimate conclusions.  Both sides declined Rule 37 motions.  The judge denied the plaintiff's Rule 56 motion at 05:16:33 UTC, finding competing reasonable inferences about offensive character and intent at commencement.  The manuscript now includes production, admissions, the motion and opposition, the reply, and the ruling.  The lawyer token-price estimate reached $81.04 at 05:15 UTC.  The case remains in progress with its inputs and runtime unchanged.

Jury selection began with thirteen candidates at 05:18 UTC.  J1, Kimi K2.6 through SiliconFlow, made 23 work-note calls without arguments before reading the MCP scripting skill and submitting its questionnaire through `mcpScript`.  It completed after 27 model responses.  Read-only inspection of the installed MCP adapter, Pi's `api/constrained-sampling.js`, and the gateway found that the argument schema permits objects and strings and that returned argument strings pass through unchanged.  No prompt or process intervention was required for J1's eventual submission.  J3, Qwen Plus 2025-07-28 through Alibaba, received an empty-name tool call followed by an upstream HTTP 400 rejecting that name in the request history.  ADC recorded the failed candidate and added J14, Qwen3 VL 235B A22B Thinking.  J2 and J4–J11 completed their questionnaires.  J12, GPT-4 Turbo, remains in its turn with repeated malformed JSON strings at 05:29 UTC.  These errors occur in the Pi adapter before an ADC decision reaches the court.  Its deadline remains in force.  No gateway history-comparison error has occurred in the fresh case.

J12 stopped at 05:33 UTC after 29 rejected JSON strings, repeatedly adding an extra closing brace.  Its final response attributed the failure to unclear formatting or parsing, although the tool results identified the JSON errors.  ADC replaced it with J15, DeepSeek V4 Flash.  MiMo V2.5 completed the next questionnaire, leaving the two replacements at 05:35 UTC.  The eleven accepted questionnaires affirm impartiality and ability to follow instructions, with little substantive variation.  The report records both tool-call failures and successful submissions, and identifies the launch prompt's requirement to encode tool arguments as strings.  The lawyer token-price estimate was $136.79 at 05:33 UTC.  The last disk check showed 5.3 GiB free, and this case occupied 131 MiB.

## ADC invalid-decision termination: 2026-09-19

Both replacement candidates completed questionnaires by 05:36:05 UTC, leaving thirteen eligible candidates.  The plaintiff proposed a bias-and-instructions question to J1, and the judge allowed it.  During turn 62, J1 submitted a work note but then called `adc_submit_decision` three times without arguments.  Each rejection reported `submit_decision kind must be tool or pass`.  The third exhausted the three-attempt allowance.  The case exited with status 1 at 05:40:23 UTC, before jury empanelment or a verdict.  The event database ends at event 90, J1's context read.  Its process log retains the rejected submissions and returned errors.

The cause is in `adc/runtime/runner/roleapi.go`.  `rejectDecisionLocked` calls `finishTurnLocked` with the exhausted-attempt error.  `executeExternalOpportunityTurn` returns that error to the case loop, which exits.  By contrast, both `failLocked` and `expireTurnLocked` invoke `handleOpportunityResponseError` for jurors.  The handler recognizes questionnaire and follow-up opportunities in voir dire and applies the existing candidate-replacement actions.  That route replaced J3 and J12 in this run.  The invalid-decision path bypasses it.  No Lean rule or proof changed during diagnosis.

The proposed correction routes exhausted juror decision attempts through the existing juror-failure handler.  It retains the three-attempt allowance, the 25-minute deadline, candidate replacement during voir dire, and the existing deliberating-juror failure policy.  Other participants retain their current error behavior.  Approval is pending for that change, its live test, and another fresh full case.  No further run has started.

The case directory remains at `adc/out/ex04-20260919-02/` and occupies 147 MiB.  The lawyer logs contain 362 completed assistant responses and $161.02 in token-price estimates, excluding court requests and search subrequests.  Podman reports no running containers and no retained ADC case containers.  No case process or staged authentication, capability, or MCP configuration file remains.  The filesystem has 5.9 GiB free.  The report now records the stopped case and remains an incomplete, unsubmitted draft.

The user approved the correction, live testing, and another fresh nine-juror/six-vote case.  `rejectDecisionLocked` now invokes the existing juror-failure handler after the third rejected decision.  Errors from replacement or removal propagate to the case loop.  The participant still receives `attempts_exhausted`, and other roles retain their existing error behavior.  The focused test submits empty decisions for a questionnaire, a follow-up, a deliberation vote, a missing juror, and a lawyer.  It checks the attempt count, replacement, reduced-jury verdict, and error propagation using the compiled Lean engine.
...
The plaintiff imported Syria's A/79/687–S/2024/888 before serving disclosures, then imported the Secretary-General's UNDOF report S/2025/154.  Its analysis file became `file-0004`, and ADC accepted TR-P1 at 08:07:15 UTC.  The report distinguishes dated December observations from the later report's February totals, treats offensive character and intent as inferences, and includes Israel's temporary-and-defensive account.  It identifies missing operational orders and coordinate analysis and the use of a reproduced prime-ministerial statement.  Its two `source_check` calls returned unresolved results.  The lawyer installed Poppler and extracted both official PDFs.  The imported PDFs match the workspace files.  The manuscript now covers disclosures, collection, and TR-P1.

At 08:10, the defense recorded a plan to import Israel's December 9 Security Council letter and test the plaintiff's interpretation against the full UNDOF report.  Its preceding note was at 08:00:36 during disclosures.  Its process log records research during that interval.  The total lawyer token-price estimate reached $5.56 at 08:10 UTC.

An early plaintiff MCP wait call supplied `after_version` and received `unexpected argument`.  The agent corrected its call to use `timeout_ms` and continued.  The ADC launcher prompts instruct the caller to supply `after_version`, whereas the installed MCP tool advertises only `timeout_ms`.  This prompt/API mismatch is recorded for review after the case.  No production prompt changed during the run.

The defense imported Israel's letter S/2024/887 and filed TR-D1 at 08:13:32 UTC.  It accepts entry and substantial evidence of non-Golan ground-holding, while contesting offensive classification and treating control as an incidental means of security rather than the required objective.  Its characterization of the Israeli letter as denying control intent goes beyond the letter's express statements and remains interpretive.  The plaintiff responded to the defense's sourcing criticism by importing the AP/PBS article as `file-0007`.  Both parties served five interrogatories.  The manuscript now covers both reports and those requests.  At 08:17, the case occupied 26 MiB and the two lawyer containers used about 450 MB combined.  The lawyer token-price estimate was $10.21.

ADC accepted interrogatory responses at 08:19:39 and 08:21:19 UTC.  The plaintiff identified the source universe, distinguished AP republication from independent reporting, and disclaimed a complete consensus count.  The defense accepted the reported UNDOF observations, acknowledged the force of fortification and duration evidence, and made no affirmative Golan-location or dated mission-change contention.  Ten plaintiff production requests and eleven defense requests followed.  The plaintiff also imported a BBC article and a GlobalSecurity reproduction of the December 17 PMO release.  The manuscript records the interrogatory answers.

During production, the plaintiff's provider rejected a request for exceeding the context window.  Pi's built-in compaction completed without intervention.  It reports 924,652 tokens before compaction and an estimated 15,142 afterward.  The plaintiff then produced `file-0009` at 08:28:49 UTC.  Recent pre-compaction requests reported over 750,000 cached input tokens, explaining part of the rising estimate.  At 08:29, the two lawyer logs recorded 254 completed assistant responses, one compaction, and $64.02 in token-price estimates.  This total includes the compaction's $0.53 estimate and excludes court and search-subrequest costs.  The case occupied 57 MiB at 08:26.  No process, prompt, or setting changed.

The plaintiff's itemized production response was accepted at 08:32:24 UTC.  A workspace search prompted import and production of the AP December 9 article as `file-0010`.  The lawyer identified a purported UNDOF capture as a 403 error page and excluded it as substantive evidence.  Its response lists eight produced files and acknowledges missing internal orders, source observation logs, coordinates, complete market/oracle history, and external definitional authority.  Defense production followed.

Repeated production passes have a confirmed engine cause.  In `adc/engine/ADC/Core.lean`, `updateCase` clears `passed_opportunities`, while `recordOpportunityPass` retains passes only until a later case update.  Each accepted production therefore makes remaining optional production candidates available again.  The logs show repeated passes on plaintiff production of files 0005–0006 and defense production of files 0001, 0004, and 0006.  This consumes model calls and turns.  The engine and prompts remain unchanged for this run.

The defense also received a context-window rejection.  Its built-in Pi compaction completed, reporting 926,846 tokens before and an estimated 14,946 afterward.  ADC accepted the next defense pass at 08:39:29 UTC.  At 08:40, lawyer accounting showed 379 completed assistant responses, two compactions, and a $129.82 token-price estimate.  These subscription-session estimates do not establish charges.  The case occupied 98 MiB at 08:37, the two containers used approximately 560 MB combined, and the filesystem had 5.3 GiB free.

ADC accepted the defense's itemized production response at 08:47:20 UTC.  It identifies seven produced files and acknowledges the same principal source-record gaps as the plaintiff.  Events 29–93, from first production through that response, contain fifteen production acts, forty-seven passes, one additional source import, and two itemized responses over 23 minutes 3 seconds.  This is a bounded interval count, since optional production opportunities remain available during pretrial.  Lawyer accounting at 08:47 recorded 457 completed responses, two compactions, and a $172.38 estimate.  The manuscript now records the production results, repeated-pass mechanism, and both compactions.

The plaintiff served twelve admission requests at 08:49:36 UTC.  Its final filed request combines offensive character and control intent, despite the preceding work note describing two separate ultimate requests.  The defense served its set at 08:53:13 UTC.  Optional production passes recur between these filings.  The plaintiff completed a second native Pi compaction, from 928,392 tokens to an estimated 16,012.  At 08:53, the lawyer logs contain 515 completed responses, three compactions, and a $202.78 estimate.

Work-note responses contribute to repeated context.  `send_work_notes` in `adc/runtime/runner/roleapi.go` calls `opportunityPayloadLocked`, which includes `turn.view` even with `includePrompt=false`.  Matching `tool_execution_start` and `tool_execution_end` by call identifier confirms that the latest two plaintiff note acknowledgments each contain 51,090 characters.  The responses include the role-visible case state.  No API or prompt change has been made during this case.

Both admission sets have responses: plaintiff at 08:57:24 UTC, defendant at 09:00:06 UTC.  The plaintiff admitted the source limitations with qualifications about their significance.  The defense admitted reported conduct and at least some non-Golan ground-holding, qualifying precise geography and denying the ultimate compound proposition.  The plaintiff then declined Rule 37 relief at 09:02:20 UTC because the defense had answered and produced controlled materials, leaving a merits disagreement for Rule 56 or trial.  The manuscript now covers the admissions and that decision.  The defense completed a second native Pi compaction, from 923,367 tokens to an estimated 14,968, before its pass at 08:56:21 UTC.  At 09:01, lawyer accounting showed 596 completed responses, four compactions, and a $231.95 estimate.  The case occupied 169 MiB at 09:00.

The defense also declined Rule 37 relief at 09:04:20 UTC.  Its reason identifies no responsive record being withheld and leaves the remaining disputes to weight and inference.  Both passes are included in the manuscript.

The plaintiff declined Rule 56 relief at 09:06:05 UTC.  It identified offensive character and control purpose as disputed material inferences that a reasonable jury could resolve for the defense and considered partial relief on admitted facts redundant.  The manuscript records that choice.  At 09:06, lawyer accounting showed 651 completed response events, four completed compactions, and a $267.92 estimate.  A third plaintiff compaction began while it was handling the recurring file-0005 production opportunity.  The case remains active, with no runtime or prompt edits.

The third plaintiff compaction completed, reporting 926,512 tokens before and an estimated 16,287 afterward.  Its next pass was accepted at 09:08:42 UTC.  The defense declined Rule 56 relief at 09:09:55 UTC on the ground that both sides had evidence supporting competing reasonable inferences.  The judge entered the pretrial order at 09:11:30 UTC, carrying forward the admitted facts and their qualifications, defining the remaining issues, and identifying ten potential exhibits subject to later objections and rulings.  The manuscript includes both decisions.  Lawyer accounting at 09:10 recorded 695 completed response events, five compactions, and a $289.21 estimate.

The defense completed its third native Pi compaction before the final pretrial pass at 09:15:20 UTC.  Trial and jury selection followed with the approved nine-juror, six-vote policy.  The initial panel contained thirteen candidates.  J2 (Seed 1.6 Flash), J3 (Qwen3 VL 8B Thinking), J4 (MiniMax M3), and J17 (Aion 2.0) exited without completing questionnaires.  J2 returned a prose case summary, J3 and J17 ended without a substantive final response, and J4 made malformed MCP calls.  J11 (Qwen3.5 27B) timed out with an outstanding provider request.  ADC replaced all five.  J5 (MiMo V2.5) completed its questionnaire but timed out on the plaintiff's follow-up question and was also replaced.  These failures used the existing timeout or process-exit handling.  They do not establish another live exercise of the corrected invalid-decision path.

The run contains fourteen completed questionnaires, twelve proposed follow-up questions, eleven completed follow-up answers, and six candidate replacements.  Both parties declined challenges for cause.  ADC accepted the plaintiff's peremptory strike against J1 at 12:34:20 UTC.  The reason identified J1's stated weighting of direct evidence as a concern for the plaintiff's circumstantial-intent theory, despite J1's assurance that it could follow a circumstantial-proof instruction.  At 12:51:43, the defense recorded its intended strike against J9 because its answers supplied less explanation of the evidentiary distinctions than other candidates' answers.

The case stopped before that defense strike was submitted.  The defense session contains a `terminated` model response at 12:51:06, then successful context reads and the final work-note call.  Its process log contains no subsequent `adc_submit_decision` call.  The core exited at turn 225 with `defendant opportunity timed out after 25m0s`.  The launcher recorded status `error` at 13:08:33 UTC.  The database ends at event 309, and the core did not write a final `run.json`, case-state file, or replay certificate.  There is no verdict.  No further case has started.
...
The user supplied the macOS host's power log at `/media/psf/Downloads/pmset.txt`.  Its timestamps use UTC−05:00.  Line 7721 records idle sleep at 04:21:50 local, or 09:21:50 UTC.  Repeated sleep and maintenance-wake cycles follow until full wake at 09:04:17 local.  The 04:56:48 and 05:38:57 maintenance wakes precede the recorded J11 and J5 timeout actions by one and nine seconds.  Lines 8006–8030 record sleep at 07:35:05, a 45-second wake starting at 07:50:07, and sleep again from 07:50:52 through 08:07:43.  The final defense work-note acknowledgment contains the deadline `2026-09-19T12:59:20Z`, or 07:59:20 local, inside the second sleep interval.  The launcher exited at 08:08:33 local, fifty seconds after the next wake.  Host sleep therefore accounts for interruptions spanning the final defense deadline, as well as the earlier clock differences.  The final guest work-note timestamp falls inside the host's recorded sleep interval, so cross-machine event ordering must allow for guest clock correction after wake.

Lines 3–4 record Parallels Toolbox releasing its display-sleep and system-sleep prevention requests at 00:00:03 local.  The log identifies the subsequent 04:21:50 sleep as idle sleep.  The VM has active `prltoolsd` and `prltimesync` processes, while `timedatectl` reports NTP inactive.  [Parallels' sleep documentation](https://kb.parallels.com/en/123111) states that Mac sleep stops VM execution.  Another full run requires a host that stays awake throughout the case.  No host power setting, ADC deadline, or runtime code has changed.

## Fresh Israel–Syria ADC case: 2026-09-19, 14:32 UTC

The user approved another fresh run.  `ex04-20260919-04` began at 14:32:57 UTC under source revision `87de350`, using the existing commands built from unchanged executable source at `dbde6f0`.  Its settings differ from run 03 only in the output and agent-state directories.  The original market document, production prompts, and installed pool are unchanged.  The generated scenario confirms nine jurors, six required votes, and 500 turns.  Both runtime deadlines are 1,500 seconds, and the invalid-attempt allowance remains three.  Both new Pi session headers confirm `openai-codex`, `gpt-5.6-sol`, and `xhigh`.  The launcher reports web search enabled and neither lawyer resumed from a previous case.  The user was reminded that the Mac must stay awake throughout the run.

The defense declined Rule 12 relief, filed its answer, and preserved the plaintiff's burden on the compound proposition.  The court selected jury trial and moved to pretrial.  At 14:34, the plaintiff was preparing initial disclosures.  Lawyer accounting recorded eighteen completed response events and a $0.55 token-price estimate.  The manuscript now follows run 04.  The preceding draft is retained at `adc/out/ex04-20260919-03/report-draft.tex` and in Git history.

Both initial disclosures were accepted by 14:34:48.  The plaintiff researched and imported Syria's S/2024/888, Israel's S/2024/887, the December 13 UNDOF statement, and the December 17 AP/PBS article.  It installed Poppler and extracted the official PDFs.  Direct AP access returned 403, and YouTube transcript extraction failed.  The technical analysis became `file-0006`, and ADC accepted TR-P1 at 14:46:12.  It distinguishes observed conduct from inferred offensive character and intent, preserves the temporary/defensive explanation, and acknowledges absent independent geolocation and audiovisual authentication.  The defense then began its source review.  The manuscript now covers collection and TR-P1.  At 14:45, lawyer logs contained 88 response events, no compactions, and a $3.79 token-price estimate.  The last disk check showed 4.6 GiB free.  Production code, prompts, and settings remain unchanged.

ADC accepted TR-D1 at 14:49:48.  The defense accepts deployment and evidence of the later fortified hold, while distinguishing security purpose from territorial-control intent at commencement.  Its sensitivity analysis acknowledges that an interpretation covering temporary security possession favors the plaintiff.  The report became `file-0007`.  The manuscript now covers both reports.  At 14:48, the retained run occupied 22 MiB, and the two lawyer containers used about 512 MB combined.

Both sides served five interrogatories and answered without objections.  ADC accepted the answers at 14:52:09 and 14:53:19.  The plaintiff disclaimed a known operational order, specific strike-to-ground-seizure link, exact coordinates, or imported multi-outlet reporting consensus.  The defense admitted reported summit geography and physical interim control, identified no contrary location source, and preserved the commencement-purpose distinction.  The manuscript now covers those answers.  At 14:54, lawyer logs recorded 153 response events, no compactions, and a $12.52 token-price estimate.  Source research and technical reports were committed and pushed as `7ac282c`.

Each side served eight production requests.  The plaintiff imported a full PBS-page package as `file-0008` in response to the defense's request for the complete source.  Its metadata identifies a new retrieval during discovery at 14:58, after preparation of the filed excerpt.  The package contains raw HTML, response metadata, and the excerpt.  Production acts repeatedly reset prior passes, as `updateCase` clears `passed_opportunities`.  At 15:00, this interval contained three production acts and nine passes.  The plaintiff later broadened its responsiveness reading and produced previously passed files 0001 and 0002.

The plaintiff received a context-window rejection, and Pi's built-in compaction completed without intervention.  It reports 923,861 tokens before and an estimated 15,524 afterward.  ADC accepted the next production at 15:02:44.  Recent pre-compaction requests exceeded 600,000 cached input tokens.  Work-note acknowledgments include the role-visible state: `send_work_notes` calls `opportunityPayloadLocked`, which assigns `view` even when `includePrompt` is false.  At 15:03, lawyer accounting recorded 245 response events, one completed compaction, and a $63.22 token-price estimate.  These subscription-session estimates exclude court and separate search costs and do not establish charges.  The case occupied 50 MiB at 14:59.  Production code and prompts remain unchanged.

The defense imported its source downloads, extracted text, UNDOF capture, and methods manifest as `file-0009`.  ADC accepted its itemized production response at 15:10:20.  It identifies nine produced files, no additional operational orders or geographic/visual evidence, and a narrow work-product objection for unfiled attorney mental impressions.  The manuscript covers both production responses, the repeated-pass behavior, and the first compaction.  At 15:09, lawyer accounting recorded 318 response events, one compaction, and a $100.12 estimate.  The retained case occupied 77 MiB at 15:06, when the filesystem had 3.8 GiB free.

The plaintiff admitted all eighteen defense admission requests at 15:13:14, with qualifications preserving its circumstantial case.  The defense admitted all twelve plaintiff requests at 15:16:25, qualifying authenticity, continuity of purpose, and abstract possibilities.  It admits that defensive motive, temporary duration, and absence of resistance do not categorically defeat the proposition.  The defense completed its first Pi compaction before responding: 923,038 reported tokens before, an estimated 15,525 afterward.  At 15:16, lawyer accounting recorded 376 response events, two compactions, and a $140.97 estimate.  The preceding discovery manuscript and PDF were committed and pushed as `d8a1e74`.

Both parties declined Rule 37 relief because the record showed no concrete unresolved discovery failure.  Both declined Rule 56 relief because offensive character and intent at commencement remained subject to competing reasonable inferences.  The judge's pretrial-order opportunity followed at turn 82.  The manuscript records those choices.

The judge entered the pretrial order at 15:17:44 and advanced to trial.  The order carries forward admissions with qualifications, identifies nine anticipated exhibits, and preserves offensive character, commencement intent, continuity, and source weight for trial.  ADC assembled thirteen initial jury candidates.  Candidate checks logged upstream 429 responses for GLM 4.7 and Gemma 4 26B configurations and used configured retries.  J1 (Muse Glimmer 30B), J2 (GLM 4.7), and J4–J8 completed questionnaires.  J3 (GPT-4.1 Nano) ended by asking whether to proceed instead of submitting its questionnaire.  ADC replaced it with J14 (Qwen3 VL 30B A3B Instruct).  J9 (Seed 2.0 Lite) is making malformed JSON-string calls rejected by Pi's MCP adapter before they reach ADC.  Its deadline remains in force.  At 15:20, lawyer accounting recorded 401 response events, two compactions, and a $151.16 estimate.  The case occupied 131 MiB at 15:22.

J9 completed its questionnaire at 15:22:47 after four malformed JSON-string calls.  J10–J12 also completed.  J13 (Qwen Plus 2025-07-28 through Alibaba) returned an empty tool name, then received an upstream HTTP 400 rejecting that name in its request history.  ADC replaced it with J15 (Mistral Small 3.2).  J14 is repeatedly sending questionnaire JSON with a missing final closing brace.  Its calls fail in the Pi MCP adapter before ADC's decision-attempt limit applies.  Its 25-minute deadline is approximately 15:49.  No runtime or prompt change has been made.  At 15:32, lawyer logs recorded 445 response events, two compactions, and a $169.42 estimate.  The lawyer wait loops continue making model requests while neither lawyer has a court turn.

Free filesystem space fell from 4.7 GiB at launch to 2.3 GiB at 15:30.  The run occupied 142 MiB at 15:27, and its three active containers had approximately 214 MB of writable data.  Those files do not explain the full decline.  Podman reports 381 images and 542 containers in the account, of which three are active for this case.  No unrelated container, image, or retained case record was deleted.  A direct storage-directory size check lacked access to rootless overlay metadata, so its 14 GiB result is incomplete.  Podman's own accounting reports 12.57 GB of images and 591 MB of container writes.  The open-deleted-file check found retained executable inodes and small temporary files, without evidence identifying the ongoing growth source.
...
The user approved a separate counter for court submissions rejected inside Pi, with a configurable default limit of 10 per opportunity.  Locally launched ADC Pi lawyers and jurors now use this counter.  CLI execution accepts `--court-submission-error-limit`, and unified settings accept `procedures.adc.court_submission_error_limit`.  Zero selects the default.  Negative values are rejected.  ADC's decision-attempt counter remains separate, and the existing participant-failure handling determines replacement or termination.

The implementation follows the installed Pi MCP Adapter 2.27.0 source in `index.ts` and `proxy-modes.ts`, documented in the [Pi container guide](containers/pi/README.md).  The adapter accepts object arguments or a JSON-encoded object string.  Malformed strings throw before dispatch, while an unknown tool returns `details.error: tool_not_found` with `isError: false`.  The launcher associates execution-start and execution-end events by call ID and counts these local submission errors.  Court reads, work notes, other local tools, transport failures, and responses returned by ADC retain their existing handling.  A lawyer's counter follows the Role API's opportunity ID and state version.  A juror process already belongs to one opportunity.

- [x] Test event classification, output preservation, counter reset, defaults, and settings forwarding.
- [x] Run affected Go tests with the race detector and run Go vet.
- [x] Test the default limit through Pi, MCP, ADC, and the compiled Lean engine.
- [x] Test the JSON override and per-opportunity reset with a live Pi lawyer.

`adc/out/court-submission-live-20260919-01/` contains the successful juror test with Sonnet 4.6.  J1 made one malformed court read, one unknown court-submission call, and nine malformed submissions.  The launcher excluded the read, stopped J1 at ten submission errors, and reported the failure to ADC.  ADC added J7, accepted its questionnaire, and completed the test with status zero.  Certificate verification replayed 25 transitions.  The test occupies 4.4 MiB.  Core accounting reports $0.005004.  Juror request records contain usage without dollar costs.
