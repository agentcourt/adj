...
The private lawyer API uses the AAR lawyer paths and tool shapes so `adjservices` can present either procedure through the same MCP adapter.  Each lawyer can inspect immutable staged documents through list, stat, and range-read operations, and the shared verified reader rejects a changed file before returning its bytes.  The council request presents UTF-8 documents as text, images as image data URLs, and PDFs as file content items after the two accepted arguments; another binary media type fails during initialization.  Each selected member must submit one structured `demonstrated` or `not_demonstrated` vote with a rationale.
