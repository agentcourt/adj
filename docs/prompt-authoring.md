...
ADC behavior evals use the same catalog and accept the same `--prompt-dir` and repeated `--prompt-file ID=PATH` options.  A suite candidate supplied through `--opportunity-prompt-file` replaces the objective in the eval opportunity before `runtime.opportunity` and `runtime.turn` render the model request.  Rules 11, 37, and 58 require `--counterfactual-model` for candidate evaluation because their production Lean opportunities specify deterministic judge actions.  Every other suite uses its production model opportunity by default.  Each model-backed result records the selected prompt source and rendered model input.
