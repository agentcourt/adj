...
The following tables list every Simple and Quick core entry.  Paths are relative to the procedure directory supplied through `--prompt-dir`, and the conventional path prepends `prompts/simple/` or `prompts/quick/`.  The core catalogs contain six Simple entries, 23 Quick entries, 45 AAR entries, 45 AARD entries, and 211 ADC entries.  A dash in either table means that the entry accepts no replacement token.
...
The [ADC prompt catalog](../adc/docs/prompts.md) lists all 211 ADC IDs, paths, and tokens through its 67-entry fixed table, 92-name direct-tool set, and 52 role-specific or shared tool cards.  Its fixed entries include the `strategy.proposition.proponent` and `strategy.proposition.opponent` prompts, the `probe.juror.identity`, `probe.juror.tool-check`, and `probe.juror.preflight` prompts, corrections, tool-result guidance, case-file attachment text, and the three `import_case_file` schema-property descriptions.  The catalog also covers complaint and case preparation, generated roles, direct and external turns, reports, tool descriptions, and tool guidance.

ADC behavior evals use the same catalog and accept the same `--prompt-dir` and repeated `--prompt-file ID=PATH` options.  A suite candidate supplied through `--opportunity-prompt-file` replaces the objective in the eval opportunity before `runtime.opportunity` and `runtime.turn` render the model request.  Rule 58 requires `--counterfactual-model` for candidate evaluation because its production Lean opportunity specifies a state-derived judgment action.  Every other suite uses its production model opportunity.  Each model-backed result records the selected prompt source and rendered model input.
...
`adc-mcp` also has an `import-file` mode that reads a participant's file and submits it through that participant's MCP assignment.  ADC's `participant.pi`, `participant.openclaw`, and `skill.openclaw` prompts describe its invocation and the return to the opportunity loop.  The [ADC manual](../adc/manual.md#legal-tools) documents command options, launcher-provided environment variables, and caller-owned participant setup.
