...
This document defines five procedure interfaces: simple, quick, AARD (`aard`), AAR (`aar`), and ADC (`adc`).  `adj` owns each procedure's case state, input custody, participant opportunities, local and remote participant launch, durable record, terminal result, and one-case process supervision.  The optional `adjservices` repository owns managed multi-case admission, public routing, deployment, web applications, and reporting, and it executes installed `adj` commands.

An interface edit requires tests for the affected `adj` command and corresponding documentation edits.  An `adjservices` integration test identifies the exact installed commands and source tree under test.  Changes to a consumed command, private route, result field, or artifact require a corresponding service edit.

## Executables and Process Behavior

The core executables are `adc`, `aar`, `aard`, `simple`, and `quick`.  Each invocation receives an explicit output directory and, for a live participant API, an explicit listen address selected by its caller.  The `adjudicate`, `adc-run`, `aar-run`, and `aard-run` commands in this repository start the required core, MCP, lawyer, council, or juror processes and supervise one complete case.  A managed service invokes those commands with explicit paths and working directories, records their standard streams, and reconciles their terminal records.
...
| Procedure | Direct one-case invocation | Required caller-facing flags |
| --- | --- | --- |
...
| `local-run.json` | `adj` local-agent orchestration record in the launcher run directory. |
...
`adj` tests verify core argument construction, startup, failure, process cleanup, record reconciliation, private API startup, direct cases, and terminal records.  Unified-command tests cover five-way settings and registration, AAR, AARD, and ADC request and result mapping, and simple and quick process arguments, participant supervision, cancellation, and record freshness.  Service integration tests invoke selected `adj` binaries and verify managed process and artifact behavior.

The `service/compat/adc`, `service/compat/arb`, and `service/compat/arbd` packages in `adjservices` exercise managed Clerk programs against selected `adj` binaries.  The ADC package completes a bench case with external parties, an internal judge, digest generation, terminal record reconciliation, and certificate replay.  The ARB and AARD packages cover participant failures, deadlines, complete MCP cases, terminal records, and service reconciliation.  All three packages accept explicit service binary, `adj` binary, and checkout paths through test flags.  Simple and quick use the unified-command path described above.

Changes to command names, required flags, private routes, consumed request or response fields, exit behavior, or record names require coordinated edits in each consumer.  The `adj` and `adjservices` documentation must describe the same installed-command interface.  The affected command and service integration tests must pass before release.
