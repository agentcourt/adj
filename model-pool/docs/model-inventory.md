...
`tools/run_model_screen.py` checks every inventory row before endpoint evaluation.  Its direct check uses the Quick council preflight request and requires one `submit_council_vote` function call.  Its Pi check uses the Pi container, MCP proxy extension, and local model gateway used by ARB, ARBD, and ADC, requiring `wait_for_opportunity` and `submit_council_vote`.  Both checks use the shared model executor and retain the inventory row's model and provider route.  The screen rejects metadata that lacks text input, text output, or tool support, and it rejects a runtime configuration unless both checks pass.  After MCP accepts the Pi vote, the screen allows five seconds for Pi to exit and then stops the test container because the council opportunity is complete.  The screen retains request status, response IDs, MCP calls, Pi output, provider-reported token usage, and observed cost.
