...
An explicit `--council-pool` selects that file.  Without that option, Quick selects `./pool.jsonl` when it exists, then `<common-root>/data/personas/pool.jsonl`.  `--common-root` overrides the shared `common` directory, whose default comes from the working directory and its ancestors.  Repeated `--council-endpoint` flags restrict selection to the named endpoints.  `--minimum-distinct-council-endpoints` requires that many endpoint names in the completed council.
...
Council requests run sequentially by default.  `--parallel-council` starts all selected council requests together.  In either mode, a provider failure, member deadline, or exhausted invalid-response limit removes that member from voting after the configured request attempts finish.  Quick records the failure and continues with the other members.  Prompt rendering, document verification, record-writing, parent-context cancellation, and other procedure errors fail the run.  Parallel mode cancels outstanding requests after one of those procedure errors.

The constituted vote requirement remains fixed after a member failure.  The council finishes after every seat has produced either a vote or a failure.  Quick returns `demonstrated` or `not_demonstrated` when one side reaches the configured vote requirement, and `no_majority` otherwise.  Parallel mode records votes and failures in council-roster order rather than response-arrival order.

Quick excludes a pool record before selection when its endpoint metadata states that the model does not support `tools`, which the vote request requires.  It selects among the least-used endpoints and then the least-used configurations, using cryptographic randomness for ties.  Each candidate must return a valid `submit_council_vote` call in a bounded availability request.  A request, protocol, transient, or request-specific authentication failure advances to the next candidate.  A missing endpoint credential excludes the remaining candidates for that endpoint while candidates for other endpoints remain eligible.  `council_candidate_rejected` events record the safe route identity of each failed candidate and its replacement, and provider accounting includes every availability request.
