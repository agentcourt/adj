...
Council records contain the selected endpoint/model reference, persona filename, and available provider token usage and cost for each vote.  The terminal result includes aggregate council usage or `council_cost_usd` only when every successful council response supplied the corresponding data.  Provider request headers remain in memory for the provider call and do not appear in the durable council, transcript, event, or run records.
