...
The [working LaTeX source](paper.tex) describes ADC, the shared model pool, jury selection, Lean procedure, and the Israel–Syria case under `adc/out/ex04-20260919-01/`.  The report remains an incomplete draft.  The run stopped during jury selection after two model-gateway compatibility failures.  It produced no verdict or completed replay certificate.  No ADC report has been submitted to marXiv.

The input settings and market-rules document are under `input/`, the unified case record is under `case/`, and retained lawyer state is under `agents/`.  The case began on September 19, 2026, at 00:34:39 UTC with source revision `543624c` and rebuilt Go commands.
