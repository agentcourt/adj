...
func Run(ctx context.Context, opts Options) (result Result, err error) {
	...
	if err != nil {
		return Result{}, fmt.Errorf("split ARB prompt files: %w", err)
	}
	opts.PromptFiles = corePromptFiles
	launcherPrompts, err := launcherprompt.Resolve("arb", opts.LauncherPromptDir, opts.LauncherPromptFiles)
	if err != nil {
		return Result{}, err
	}
	...
	if err != nil {
		return Result{}, fmt.Errorf("generate ARB MCP signing key: %w", err)
	}
	...
	if err != nil {
		cancel()
		return Result{}, fmt.Errorf("start ARB MCP: %w", err)
	}
...
func coreCaseArgs(opts Options, caseAPIAddr string) []string {
	...
	addInt("--required-votes", opts.RequiredVotes)
...
func applyDefaults(opts Options) Options {
	...
	if strings.TrimSpace(opts.CoreOutputDir) == "" && strings.TrimSpace(opts.OutputDir) != "" {
		opts.CoreOutputDir = filepath.Join(opts.OutputDir, "aar-output")
	}
...
func hasAutomaticLawyerRunner(opts Options, runner LawyerRunner) bool {
	for _, role := range autoLawyerRolesOrEmpty(opts.AutoLawyers) {
		if lawyerProfile(opts, role).Runner == runner {
			return true
		}
	}
	return false
...
func validateLawyerProfile(profile LawyerProfile, opts Options, baseEnv []string) error {
	baseEnv = lawyerEnvironment(profile, opts, baseEnv)
	...
	case LawyerOpenClaw:
		...
		_, err := resolveOpenClawLawyerAuth(profile, opts, baseEnv)
		return err
	case LawyerPi:
		if strings.TrimSpace(profile.Command) != "" {
			return errors.New("Pi uses the configured Podman command and Pi image")
		}
		return headless.ValidateProfile(headlessProfile(profile, opts), baseEnv)
...
func (s *runState) issueMCPCapability(assignmentType, principalID string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("issue ARB MCP capability for %s %s: %w", assignmentType, principalID, err)
	}
...
func headlessMCPServerName(caseID, role string) string {
	name := containerName("aar-" + caseID + "-" + role)
...
func (s *runState) startPiCouncil(ctx context.Context, entry councilRosterEntry, mcpPort string, opportunityID string) error {
	...
	proc, err := s.startProcess(ctx, processName, "podman", s.opts.PodmanCommand, args, name, &councilProcessTarget{
		memberID:      entry.MemberID,
...
func piContainerName(caseID string, memberID string, opportunityID string) string {
	return containerName("aar-" + caseID + "-" + councilProcessName(memberID, opportunityID))
}
...
func (s *runState) startProcess(ctx context.Context, name string, kind string, command string, args []string, container string, councilTarget *councilProcessTarget) (*processRecord, error) {
